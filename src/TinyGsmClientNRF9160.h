...
class TinyGsmNrf9160 : public TinyGsmModem<TinyGsmNrf9160>,
 ...
 protected:
  ...
  bool modemConnect(const char* host, uint16_t port, uint8_t mux,
    ...
    if (request_op != result_op || request_type != result_type || request_role != result_role) return false;
  ...
  int16_t modemSend(const void* buff, size_t buff_len, uint8_t mux) {
    ...
    for(uint8_t j = 0; j < (block_count + 1); j++) {
      ...
      while(1) {
        ...
        stream.write("AT#XSEND=0,\""); //0 = hex data, 1 = text data
        //Serial.print("j = ");
        //Serial.println(j);
  ...
  size_t modemRead(size_t size, uint8_t mux) {
    ...
    int timeout_s = 5;
    uint32_t timeout_ms = ((uint32_t)timeout_s) * 1000;
    sendAT(GF("#XRECV="), (uint16_t)size);
    if (waitResponse(timeout_ms, GF("#XRECV:")) != 1) { return 0; }
